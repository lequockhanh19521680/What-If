  ...
  try {
    ...
    if (!event.body) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: "No input specified" }),
      };
    }
    ...
    const imagesData = await aiService.generateMultipleImages(
      scenarioData.images
    );

    // Step 3: Upload images to S3
    console.log("Uploading images to S3...");
    const uploadedImages = await mediaService.uploadMultipleImages(
      imagesData,
      projectId
    );
    ...
    const videoData = await mediaService.createVideoSlideshow(
      imageUrls,
      projectId
    );
